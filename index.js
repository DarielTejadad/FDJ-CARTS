...
const config = {
    ...
    channels: {
        ...
        announcements: process.env.ANNOUNCEMENTS_CHANNEL_ID // Nuevo canal para anuncios
    },
    economy: {
        startingMoney: 100, dailyReward: 50, maxGiftAmount: 1000, maxDuelBet: 500,
        loanInterest: 1.1, // 10% de interés
        workReward: 25,
        workCooldown: 3600000 // 1 hora
    },
    cooldowns: {
        default: 5000, daily: 86400000, work: config.economy.workCooldown
    },
    events: { // Sistema de eventos globales
        doubleDrops: false,
        doubleDaily: false,
...
// ========================================
// CONEXIÓN Y PREPARACIÓN DE LA BASE DE DATOS (SQLite Expandida)
// ========================================
...
function initializeDatabase() {
    ...
    // (La lógica de creación de tablas es la misma, pero con más tablas)
    tables.forEach(sql => db.run(sql));
    // Poblar logros si no existen
...
// ========================================
function ensureUserExists(userId, username) { /* (Sin cambios) */ }
function getUserData(userId) { /* (Sin cambios) */ }
function updateUserData(userId, fields) { /* (Sin cambios) */ }
function getCardData(cardId) { /* (Sin cambios) */ }
function getAllCards() { /* (Sin cambios) */ }
function getShopCards() { /* (Sin cambios) */ }
function addTransaction(userId, type, amount, reason) { /* (Sin cambios) */ }
function getRarityData(rarity) { return config.bot.rarity[rarity] || { emoji: '❓', color: 0x000000, sellPrice: 5, enchantCost: 100 }; }
        ...
        achievements.forEach(achievement => {
            db.get('SELECT * FROM user_achievements WHERE user_id = ? AND achievement_id = ?', [userId, achievement.id], (err, row) => {
                ...
                if (!row) {
                    ...
                    userCache.delete(userId); // Invalidar caché
                    // Aquí se podría enviar un DM al usuario notificándole del logro
...
// ========================================
// SISTEMA DE EMBEDS CON BRANDING (Mejorado)
// ========================================
...
const achievementsCommand = {
    ...
    async execute(interaction, client) {
        db.all(`SELECT a.*, CASE WHEN ua.user_id IS NOT NULL THEN '✅' ELSE '🔒' END as status FROM achievements a LEFT JOIN user_achievements ua ON a.id = ua.achievement_id AND ua.user_id = ?`, [interaction.user.id], (err, rows) => {
            ...
            const unlocked = rows.filter(r => r.status === '✅');
            const locked = rows.filter(r => r.status === '🔒');
            
            const embed = createEmbed({
                title: '🏆 Tus Logros',
                description: `Has desbloqueado **${unlocked.length}** de **${rows.length}** logros.`,
                color: config.bot.colors.primary
            });
            
            if (unlocked.length > 0) {
                embed.addFields({ name: '✅ Desbloqueados', value: unlocked.map(a => `**${a.name}** - ${a.description}`).join('\n') || 'Ninguno', inline: false });
            }
            if (locked.length > 0) {
                embed.addFields({ name: '🔒 Por Desbloquear', value: locked.map(a => `**${a.name}** - ${a.description}`).join('\n') || 'Ninguno', inline: false });
            }
...
const favoriteCommand = {
    data: new SlashCommandBuilder().setName('favorite').setDescription('Marca una carta como favorita').addStringOption(o => o.setName('nombre').setDescription('Nombre de la carta a marcar').setRequired(true)),
    cooldown: 10000, category: 'cards',
    async execute(interaction, client) {
        const cardName = interaction.options.getString('nombre');
        db.get(`SELECT ui.id FROM user_inventory ui JOIN cards c ON ui.card_id = c.id WHERE ui.user_id = ? AND c.name = ?`, [interaction.user.id, cardName], (err, invCard) => {
            ...
            db.run('UPDATE user_inventory SET is_favorite = 0 WHERE user_id = ?', [interaction.user.id], (err) => {
                db.run('UPDATE user_inventory SET is_favorite = 1 WHERE id = ?', [invCard.id], (err) => {
                    if (err) return interaction.reply({ embeds: [createEmbed({ description: '❌ Error al marcar la carta como favorita.', color: config.bot.colors.error })], ephemeral: true });
                    interaction.reply({ embeds: [createEmbed({ description: `✅ **${cardName}** es ahora tu carta favorita.`, color: config.bot.colors.success })] });
                });
            });
        });
    }
...
const workCommand = {
    ...
    async execute(interaction, client) {
        ...
        const workPhrases = [
            "Has trabajado como minero de cristales y encontrado gemas valiosas.",
            "Has hecho una entrega especial para el gremio de mercaderes.",
            "Has ayudado a reparar la muralla de la ciudad.",
            "Has forjado espadas para la guardia real."
        ];
        
        const embed = createEmbed({
            title: '💼 Trabajo Realizado',
            description: `${workPhrases[Math.floor(Math.random() * workPhrases.length)]}\nHas ganado **${workReward} monedas**.`,
...
const loanCommand = {
    ...
    async execute(interaction, client) {
        ...
        db.get('SELECT * FROM loans WHERE user_id = ? AND is_paid = 0', [interaction.user.id], (err, loan) => {
            ...
            const dueDate = Date.now() + (7 * 24 * 60 * 60 * 1000); // Vence en 7 días
            const totalToPay = Math.round(amount * config.economy.loanInterest);
            ...
            db.run('INSERT INTO loans (user_id, amount, interest_rate, due_date) VALUES (?, ?, ?, ?)', [interaction.user.id, amount, config.economy.loanInterest, dueDate], (err) => {
                ...
                const embed = createEmbed({
                    ...
                    description: `Has recibido **${amount} monedas**.\nDeberás pagar **${totalToPay} monedas** en 7 días.\nUsa \`/payloan\` para pagarlo cuando puedas.`,
                    color: config.bot.colors.success
                });
                interaction.reply({ embeds: [embed] });
            });
...
const enchantCommand = {
    ...
    async execute(interaction, client) {
        ...
        db.get(`SELECT ui.id, ui.enchant_level, c.* FROM user_inventory ui JOIN cards c ON ui.card_id = c.id WHERE ui.user_id = ? AND c.name = ?`, [interaction.user.id, cardName], (err, invCard) => {
            ...
            const rarityData = getRarityData(invCard.rarity);
            const cost = rarityData.enchantCost * (invCard.enchant_level + 1);
            ...
            db.serialize(() => {
                ...
                db.run('UPDATE users SET money = money - ? WHERE user_id = ?', [cost, interaction.user.id]);
                ...
                db.run('COMMIT', (err) => {
                    ...
                    userCache.delete(interaction.user.id);
                    checkAchievements(interaction.user.id, 'enchant', 1);
                    
                    const embed = createEmbed({
                        title: '✨ Carta Encantada',
                        description: `**${invCard.name}** ha sido encantada al nivel **${invCard.enchant_level + 1}**.\n\nNuevas estadísticas:\n⚔️ Ataque: ${newAttack}\n🛡️ Defensa: ${newDefense}`,
                        thumbnail: invCard.image_url,
...
const eventCommand = {
    ...
    async execute(interaction, client) {
        ...
        config.events[eventType] = state;
        const statusText = state ? 'activado' : 'desactivado';
...
const botstatsCommand = {
    data: new SlashCommandBuilder().setName('botstats').setDescription('Muestra estadísticas del bot').setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
    cooldown: 30000, category: 'admin',
    async execute(interaction, client) {
        db.get('SELECT COUNT(*) as totalUsers FROM users', [], (err, userCount) => {
            db.get('SELECT COUNT(*) as totalCards FROM user_inventory', [], (err, cardCount) => {
                db.get('SELECT COUNT(*) as totalDrops FROM card_drops', [], (err, dropCount) => {
                    db.get('SELECT SUM(money) as totalMoney FROM users', [], (err, moneySum) => {
                        const embed = createEmbed({
                            title: '📊 Estadísticas del Bot',
                            color: config.bot.colors.info,
                            fields: [
                                { name: 'Usuarios Totales', value: userCount.totalUsers, inline: true },
                                { name: 'Cartas en Circulación', value: cardCount.totalCards, inline: true },
                                { name: 'Drops Realizados', value: dropCount.totalDrops, inline: true },
                                { name: 'Dinero Total en Economía', value: `${moneySum.totalMoney || 0} monedas`, inline: true }
                            ]
                        });
...
// ... (Aquí irían todos los demás comandos anteriores como drop, claim, buy, duel, etc., sin cambios)
...
registerCommand(favoriteCommand); registerCommand(workCommand); registerCommand(loanCommand); registerCommand(enchantCommand);
registerCommand(announceCommand); registerCommand(eventCommand); registerCommand(givecardCommand); registerCommand(botstatsCommand);
// ... (Registrar también todos los comandos anteriores)

// ========================================
// EVENTOS DEL BOT (Modificado para Logros)
// ========================================
...
client.on('interactionCreate', async interaction => {
    // ... (Lógica de interacción existente sin cambios)
    // ... pero después de ejecutar un comando con éxito, añadir lógica de logros
    if (interaction.commandName === 'claim') {
        const userData = await getUserData(interaction.user.id);
        checkAchievements(interaction.user.id, 'claim', 1);
    }
    if (interaction.commandName === 'duel') {
        // Lógica para verificar victorias y actualizar logros
    }
    // ... etc.
});
